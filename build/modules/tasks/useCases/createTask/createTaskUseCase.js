...
const UsersRepository_1 = require("../../../accounts/repositories/implementations/UsersRepository");
let CreateTaskUseCase = class CreateTaskUseCase {
    ...
    async execute({ title, description, userId }) {
        ...
        if (!user) {
            throw new AppError_1.AppError("Usuario não encontra");
        }
        await this.tasksRepository.create({ title, description, userId: user.id });
    }
};
CreateTaskUseCase = __decorate([
